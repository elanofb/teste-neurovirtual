...
namespace BLL.AssociadosOperacoes.Emails  {
	...
	public class EnvioRecuperacaoSenhaTransacao : EnvioEmailAdapter, IEnvioRecuperacaoSenhaTransacao {
		...
		//Private Construtor
		private EnvioRecuperacaoSenhaTransacao(int idOrganizacaoParam, List<string> listaDestino, List<string> listaCopia, List<string> listaCopiaOculta) :
			base(idOrganizacaoParam, listaDestino, listaCopia, listaCopiaOculta) {
		}

		//Factory
		public static EnvioRecuperacaoSenhaTransacao factory(int idOrganizacaoParam, List<string> listaDestino, List<string> listaCopia, List<string> listaCopiaOculta = null) { 
			return new EnvioRecuperacaoSenhaTransacao(idOrganizacaoParam, listaDestino, listaCopia, listaCopiaOculta);
		}
		...
		protected override string capturarConteudoHTML(string arquivoHTML) {
            ...
            var conteudoHTML = this.OConfiguracaoNotificacao.corpoEmailRecuperacaoSenhaTransacaoAssociado;
		...
		public UtilRetorno enviar(NotificacaoSistemaEnvio OEnvio, string linkRecuperacao = "") {
		    ...
		    if (linkRecuperacao.isEmpty()) {
                linkRecuperacao = ConfiguracaoLinkBaseBL.linkAreaAssociado(idOrganizacao, $"minhaconta/alteracaosenhatransacao/nova-senha{ parametros }");
            }
            
            infos["link"] = linkRecuperacao;

		    string tituloEmail = this.OConfiguracaoNotificacao.tituloEmailRecuperacaoSenhaTransacaoAssociado.Replace("#NOME_ORGANIZACAO#", OConfiguracaoSistema.tituloSistema);
