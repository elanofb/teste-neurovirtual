...
namespace BLL.AssociadosOperacoes {
    ...
    public class AssociadoRecuperacaoSenhaTransacaoNotificacaoBL : DefaultBL, IAssociadoRecuperacaoSenhaTransacaoNotificacaoBL {
        ...
        public UtilRetorno  registrarEmails(List<ItemListaAssociado> listaAssociados) {
            ...
            if (listaAssociados.Any(x => x.ativo != "S" && x.ativo != "E")) {
                return UtilRetorno.newInstance(true, "Existem Mebros desativados na lista, eles não podem receber o link de recuperação de senha de transações. Tente novamente!");
            }
            ...
            if(ONotificacao.id > 0) {
                ...
                return UtilRetorno.newInstance(false, "O link de reenvio de senha foi enviado para o(s) e-mail(s) do(s) membro(s), através dele será possível criar uma nova senha de transações.");

            }
            
            return UtilRetorno.newInstance(true, "Não foi possível enviar o link de recuperação de senha de transações para os membros.");
        ...
        private NotificacaoSistema gerarNotificacao() {
            ...
            ONotificacao.idTipoNotificacao = TipoNotificacaoConst.ASSOCIADO_RECUPERACAO_SENHA_TRANSACAO;
                
            ONotificacao.titulo = OConfigNotificacao.tituloEmailRecuperacaoSenhaTransacaoAssociado.Replace("#NOME_ORGANIZACAO#", OConfigSistema.tituloSistema);

            ONotificacao.notificacao = OConfigNotificacao.corpoEmailRecuperacaoSenhaTransacaoAssociado;
